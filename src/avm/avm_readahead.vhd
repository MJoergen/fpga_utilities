...
architecture rtl of avm_readahead is
  ...
  signal cache_addr    : std_logic_vector(G_ADDR_BITS - 1 downto 0);  -- Base address of the currently cached region
  ...
  -- Loaded from s_burstcount_i on a read miss.  Decremented each
  -- cycle a word is forwarded via s_readdatavalid_o.  When a cache
...
begin
  ...
  -- Compile-Time Validation of G_DATA_BITS
  assert G_DATA_BITS >= 8
    report "G_DATA_BITS must be >= 8"
    severity failure;
  ...
  -- Unsigned modular subtraction with natural wrap-around on G_ADDR_BITS bits.
  ---------------------------------------------------------------------------
  cache_offset_s  <= std_logic_vector(unsigned(s_address_i) - unsigned(cache_addr));

  ---------------------------------------------------------------------------
  -- cache_filled_s: Asserted for exactly one cycle when the final word of
  -- a cache fill burst is received from the master bus.
  ---------------------------------------------------------------------------
  cache_filled_s  <= '1' when state = READING_ST and m_readdatavalid_i = '1' and cache_count = G_CACHE_SIZE - 1 else
                     '0';
  ...
  fsm_proc : process (clk_i)
    ...
    begin
      ...
      m_address_o    <= s_address_i;
      m_burstcount_o <= std_logic_vector(to_unsigned(G_CACHE_SIZE, G_BURST_BITS));
      rd_burstcount  <= s_burstcount_i;
      cache_count    <= 0;
      cache_addr     <= s_address_i;
      state          <= READING_ST;
  ...
  begin
    if rising_edge(clk_i) then
      if rst_i = '0' then
        assert not (s_write_i = '1' and s_read_i = '1')
        ...
        when IDLE_ST =>
          ...
          if s_read_i = '1' and s_waitrequest_o = '0' then
            if cache_rd_hit_s = '1' then
              ...
              -- received via m_readdata_i but the corresponding
              ...
              if unsigned(cache_offset_s) >= G_CACHE_SIZE / 2 - 1 then
                ...
                m_read_o                              <= '1';
                m_byteenable_o                        <= (others => '1');
                m_address_o                           <= std_logic_vector(unsigned(cache_addr) + G_CACHE_SIZE);
                m_burstcount_o                        <= std_logic_vector(to_unsigned(G_CACHE_SIZE / 2, G_BURST_BITS));
        ...
        when READING_ST =>
          if m_readdatavalid_i = '1' then
            ...
            if cache_count >= G_CACHE_SIZE - 1 then
              ...
              elsif s_write_i = '1' and s_waitrequest_o = '0' then
                ---------------------------------------------------------
