...
begin
  ...
  mem_write_address    <= s_address_i when unsigned(write_burstcount) = 0 else
                          write_address;
  mem_read_address     <= s_address_i when unsigned(read_burstcount) = 0 else
                          read_address;
  mem_write_burstcount <= s_burstcount_i when unsigned(write_burstcount) = 0 else
                          write_burstcount;
  mem_read_burstcount  <= s_burstcount_i when unsigned(read_burstcount) = 0 else
                          read_burstcount;

  s_waitrequest_o      <= '0' when unsigned(read_burstcount) = 0 else
                          '1';
      ...
      if s_write_i = '1' and s_waitrequest_o = '0' then
        ...
        if G_DEBUG then
          report "Avalon SLAVE " & G_NAME &
                 ...
                 " with burstcount " & to_hstring(mem_write_burstcount) &
